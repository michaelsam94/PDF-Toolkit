        ...
        try {
            context.contentResolver.openFileDescriptor(uri, "r")?.use { pfd ->
                ...
                for (i in 0 until renderer.pageCount) {
                    ...
                    // Render base page details
                    canvas.drawBitmap(bitmap, 0f, 0f, null)
                    
                    // Draw custom painted stroke layout if matching selected page index
                    if (i == placement.pageIndex) {
                        val sigW = width * placement.widthRatio
                        val sigH = sigW * (signatureBitmap.height.toFloat() / signatureBitmap.width.toFloat())
                        
                        val x = width * placement.xRatio
                        val y = height * placement.yRatio
                        
                        val destRect = android.graphics.RectF(x, y, x + sigW, y + sigH)
